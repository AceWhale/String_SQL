-- 1 
DECLARE @Palindrom NVARCHAR(MAX) = 'дед боб дед' -- нет больше деда Ѕоба

IF @Palindrom = REVERSE(@Palindrom)
    PRINT '—трока палиндромом.'
ELSE
    PRINT '—трока не палиндромом'



-- 2
DECLARE @String NVARCHAR(MAX) = 'Ќажал кабан на кабана и снова кабан.'
...
PRINT @Search + '" встречаетс€ = ' + CAST(@Count AS NVARCHAR(10)) + ' раз(a) в строке'
